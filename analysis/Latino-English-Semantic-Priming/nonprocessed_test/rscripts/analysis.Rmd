...
critical_rt = d %>%
  ...
  group_by(List, Trial_Type)
ggplot(critical_rt, aes(x=Response_Time,fill=List)) +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Log RT Density Plot
ggplot(critical_rt, aes(x=log(Response_Time),fill=List)) +
  geom_density(alpha=.5) +
  xlab("Log RT") +
  ylab("Density")
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#THIS NEEDS TO BE COLLAPSED BY CONDITION EVENTUALLY
critical_rt = d %>%
  filter(Trial_Type == "control" | Trial_Type == "critical") %>%
  group_by(List, Trial_Type) %>%
  summarize(MeanRT = mean(Response_Time), CI.Low = ci.low(Response_Time), CI.High = ci.high(Response_Time)) %>%
